class Api::V1::InsightsController < ApplicationController
  ...
  def create
    ...
    insightable_type_var = User.find_by(username: params["username"]) ? User.find_by(username: params["username"]).class.to_s : Person.find(params[:id].class.to_s)
    insightable_id_var = params[:id].to_i
    insight = Insight.new(writing_sample: params[:writingSample], insightJSON: response, openness: openness_trait, agreeableness: agreeableness_trait, conscientiousness: conscientiousness_trait, introversion_extraversion: extraversion_trait, emotional_range: emotional_range_trait, insightable_type: insightable_type_var, insightable_id: insightable_id_var)
    if insight.save
